...
const ShelfBox: React.FC<ShelfData> = ({ position, rackId, id: shelfId }) => {
    const { availableSpace, activeBatchId, batchAmount, batchPosition, batchPositionSet, batchPositionDelete, batchPositionExists } = usePregerminationContext();
    ...
    const getMax = (currentlyPlaced: number): number => {
        ...
        const valueDifference: number = currentlyPlaced - batchPosition[shelfId] || 0;
        // Calculate the amount left to be placed
        const leftToBePlaced: number = batchAmount - getPlacedAmount(batchPosition) + currentlyPlaced - valueDifference;
        ...
        return leftToBePlaced < availableSpaceAtPosition ? leftToBePlaced : availableSpaceAtPosition;
    };
