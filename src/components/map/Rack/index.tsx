...
const DraggableBox: React.FC<DraggableBoxProps> = ({
  ...
  const mouseDownHandlerWrapper = function(e: React.MouseEvent<HTMLDivElement>) {
  ...
  return (
    ...
    <>
      <div 
        className={"absolute flex flex-col bg-colorprimary rounded-lg p-1 outline outline-green-800 outline-1 outline-offset-0" + (isSelected ? 'cursor-grabbing scale-105 border-black' : 'cursor-grab')}
      ...
      >
          ...
          <div className="bg-colorprimary rounded-lg mb-1">
            {/* Show rack ID */}
            {displayMode === DisplayMode.input || displayMode == DisplayMode.view ? 
                <p className="text-center text-white rounded-lg select-none"> {`Reol #${rackData.id}`} </p> 
              : displayMode === DisplayMode.edit ?
              <div className="flex flex-row items-center w-full h-fit justify-center gap-x-1.5 mt-1">
                <Minus className="stroke-white hover:cursor-pointer hover:scale-110 hover:stroke-gray-100" onClick={() => {
                ...
                <Plus className="stroke-white hover:cursor-pointer hover:scale-110 hover:stroke-gray-100" onClick={() => {
                ...
                <Trash2 className="stroke-white hover:cursor-pointer hover:scale-110 hover:stroke-gray-100" onClick={() => {
                  ...
                  if (rackShelves.length != 0) {
                    ...
                    toast({
                      variant: "destructive",
                      title: "Noget gik galt",
                      description: "Reolen skal være tom for at kunne fjerne den.",
                    })
                    return;
                  } else {
