...
import { DisplayMode, snapToGrid, CanvasComponentMethods } from "@/components/map/CanvasComponent";
...
export const EditMenu: React.FC<{
  ...
  const setMouseUp = useCallback(function(e: MouseEvent) {
    //Create new rack
    if(canvasComponentRef.current && isMouseDown){
      const {canvasRelativeX, canvasRelativeY} = getPositionRelativeToCanvas(e);
  ...
  const isMouseInsideCanvas = function(e: MouseEvent){
  ...
  const getPositionRelativeToCanvas = function(e: MouseEvent){
      ...
      return {canvasRelativeX: canvasRelativeX, canvasRelativeY: canvasRelativeY};
  }
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    if(isMouseDown && elementRef.current && canvasRef.current){
      ...
      const canvasRelativeToContainerX = canvasOffset.left - containerOffset.left;
      const canvasRelativeToContainerY = canvasOffset.top - containerOffset.top;

      setPosition({x: canvasRelativeToContainerX + canvasRelativeX, y: canvasRelativeToContainerY + canvasRelativeY});
      ...
      <div className="grid grid-rows-4 gap-y-4 w-full items-center my-8">
        <div className="mx-5 h-full relative flex items-center justify-center"style={{height:"300px"}} >
          <div className="flex flex-col items-center w-full h-full bg-gray-400 rounded-xl text-center"> {/* Background box */}
            ...
            <div ref={elementRef} className="relative flex justify-center items-center" style={{width:"0", height:"0", zIndex:"100", left:`${-rackWidth/2}px`}}> {/* Adjusted wrapper */}
              <DraggableBox 
                rackData={{
                  id: 0,
                  position: position,
